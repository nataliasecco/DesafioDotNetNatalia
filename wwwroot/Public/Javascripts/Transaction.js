...
$(document).ready(async function () {
    ...
    function ValidateFormInput() {
        if (
            ...
            alert("Nenhuma informação pode ser cadastrada deixando vazios");
            return true;
        } else {
            return false;
        }
        ...
        try {
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            return type == "display" ?
                                `<button class="button is-success is-outlined is-rounded is-small" onclick="${JSON.stringify(row)})>Ver detalhes</button>` : data
                        }
                    },
            ...
            transactionsRowData.on("click", "button", async function () {
                ...
                deleteTransaction.on("click", async function () {
                    const confirmTransactionRemove = confirm("Deseja mesmo remover esta transação?");
                    if (confirmTransactionRemove == true) {
                        ...
                        if (removeTransaction.status == 200) {
                            ...
                            alert("Falha ao deletar a transação do banco.");
                            window.location.reload();
                        }
                        window.location.reload();
                    } else {
            ...
            console.error("Falha ao tentar carregar a tabela de transações: ", error);
            throw error.message;
        }
    }
