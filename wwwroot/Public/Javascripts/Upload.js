...
document.addEventListener("DOMContentLoaded", function () {
    ...
    saveFile.addEventListener("click", async function () {
        try {
            ...
            if (response.status == 200 && response.statusText == "OK") {
                ...
                alert("Tudo certo, dados salvos com sucesso!");
                fileInput.value = "";
                fileNameUploaded.innerText = "Nenhum arquivo foi selecionado";
                return;
            } else {
                alert(`Falha no carregamento do arquivo: ${response.statusText}`);
                return;
            }
        } catch (error) {
            console.error("Falha ao tentar enviar arquivo cnab (.txt)");
            throw error.message;
        }
    });
    ...
    cancellFileUpload.addEventListener("click", function () {
        fileInput.value = "";
        fileNameUploaded.innerText = "Nenhum arquivo selecionado";
    });
});