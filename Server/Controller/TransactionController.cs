...
namespace AplicacaoDesafioDotNet.Server.Controller.TransactionController
{
    // Classe usada para assumir a responsabilidade de controlador.
    public class TransactionController
    {
        public static string local = "[TRANSACTION-CONTROLLER]";
        public static IMongoCollection<Transaction> transactionCollection = Database.Client.GetDatabase("DesafioDotNet").GetCollection<Transaction>("dispositivos");
        ...
        public async Task<List<Transaction>> TreatCnabFile()
        {
            ...
            {
                Console.Error.WriteLine($"{local} - Falha ao tentar configurar o conteúdo do arquivo: {error}");
                throw;
            }
        }
        ...
        private List<Transaction> ParseCnabFile(string line)
        {
            ...
            {
                Console.Error.WriteLine($"{local} - Falha ao tentar analisar o arquivo cnab (.txt): {error}");
                throw;
            }
        }
        ...
        public static async Task<List<Transaction>> GetTransactions()
        {
            ...
            {
                Console.Error.WriteLine($"{local} - Falha ao tentar obter todas as transações: {error}");
                throw;
            }
        }
        ...
        public static async Task DeleteTransaction(string id)
        {
            try
            {
                if(id == null || id == "")
                {
                    Console.Error.WriteLine($"{local} - ID não definido para excluir a transação");
                    throw new Exception("ID não definido para excluir a transação");
                }
                var filter = Builders<Transaction>.Filter.Eq(query => query.Id, id);
                await transactionCollection.DeleteOneAsync(filter);
            } catch (Exception error)
            {
                Console.Error.WriteLine($"{local} - Falha ao tentar excluir a transação: ", error);
                throw;
            }
        }
        ...
        public static async Task CreateTransaction(Transaction transaction)
        {
            try
            {
                if (transaction == null)
                {
                    Console.Error.WriteLine($"{local} - A transação é necessária");
                    throw new Exception("A transação é necessária.");
                }
                await transactionCollection.InsertOneAsync(transaction);
            } catch(Exception error)
            {
                Console.Error.Write($"{local} - Falha ao tentar criar uma nova transação: {error}");
                throw;
            }
        }
